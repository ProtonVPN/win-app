...
namespace ProtonVPN.App.Test.P2PDetection.Blocked
{
    ...
    public class BlockedTrafficTest
    {
        ...
        {
            // Arrange
            var timeout = TimeSpan.FromSeconds(57);
            // Act
        ...
        public async Task Detected_ShouldBeTrue_WhenHttpResponse_ContainsPattern()
        {
            ...
            var response = HttpResponseFromString("aj shhd ajh khfk  <!--P2P_WARNING--> owjd ewh e qo");
            _httpClient.GetAsync(p2PStatusUri).Returns(response);
            var subject = new BlockedTraffic(_httpClients, p2PStatusUri, TimeSpan.FromSeconds(10));
            // Act
