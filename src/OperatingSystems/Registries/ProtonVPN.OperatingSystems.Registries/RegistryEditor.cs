...
namespace ProtonVPN.OperatingSystems.Registries
{
    public class RegistryEditor : IRegistryEditor
    {
        ...
        public object? ReadObject(RegistryUri uri)
        {
            ...
            try
            {
                key = OpenBaseKey().OpenSubKey(uri.Path);
                if (key != null)
                {
        ...
        private bool WriteObject(RegistryUri uri, object value, RegistryValueKind valueKind)
        {
            ...
            RegistryKey key = OpenBaseKey().CreateSubKey(uri.Path);
            if (key == null)
            {
        ...
        {
            bool? isSuccess = null;
            RegistryKey? key = OpenBaseKey().OpenSubKey(uri.Path, RegistryKeyPermissionCheck.ReadWriteSubTree);
            if (key != null)
            {
                try
                {
                    key.DeleteValue(uri.Key);
                    isSuccess = true;
                }
                catch (Exception ex)
                {
