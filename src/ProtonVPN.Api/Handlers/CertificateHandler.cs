...
namespace ProtonVPN.Api.Handlers
{
    ...
    public class CertificateHandler : WebRequestHandler
    {
        ...
        public CertificateHandler(Config config, IReportClient reportClient)
        {
            _config = config.TlsPinningConfig;
            ...
            X509Chain chain, SslPolicyErrors sslPolicyErrors)
        {
            ...
            TlsPinnedDomain domain = GetPinnedDomain(host) ?? GetPinnedDomain("*");
            if (domain == null)
            {
            ...
            bool valid = _policy.Valid(domain, certificate);
