...
namespace ProtonVPN.Client.Legacy.UI.Connections.Gateways;

public class GatewaysPageViewModel : CountriesPageViewModelBase
{
    public override string? Title => Localizer.Get("Gateways_Page_Title");

    public override string Description => Localizer.Get("Gateways_Page_Description");

    public override ImageSource IllustrationSource { get; } = ResourceHelper.GetIllustration("GatewaysIllustrationSource");
    ...
    protected override IEnumerable<LocationItemBase> GetItems()
    {
        return ServersLoader.GetGateways()
                            .Select(LocationItemFactory.GetGateway);
    }
}