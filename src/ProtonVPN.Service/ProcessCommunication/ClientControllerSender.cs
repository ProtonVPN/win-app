...
public class ClientControllerSender : IClientController, IClientControllerSender, IServiceSettingsAware
{
    ...
    private readonly Channel<string> _openWindowChannel = Channel.CreateUnbounded<string>();
    ...
    public IAsyncEnumerable<string> StreamOpenWindowAsync()
    {
        return StreamAsync(_openWindowChannel);
    }
    ...
    public async Task SendCurrentConnectionDetailsAsync()
    {
        await SendConnectionDetailsChangeAsync(_connectionDetails);
    }
    ...
    public async Task OpenWindowAsync(string args)
    {
        _logger.Debug<ProcessCommunicationLog>($"Sending Open Window request");
        await _openWindowChannel.Writer.WriteAsync(args);
    }
}