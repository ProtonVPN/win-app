...
namespace ProtonVPN.WireGuardService
{
    class Program
    {
        ...
        private static void LogError(string[] args)
        {
            ...
            EventLogger.LogError(_config.WireGuard.ServiceName, message);
        }

        private static string GetErrorMessage(string[] args)
        {
            StringBuilder message = new();
            message.AppendLine($"Failed to start {_config.WireGuard.ServiceName} Service due to incorrect number of arguments.");
