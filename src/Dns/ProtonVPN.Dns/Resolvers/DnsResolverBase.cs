...
public abstract class DnsResolverBase
{
    ...
    protected DnsResolverBase(IConfiguration config, ILogger logger)
    {
        Logger = logger;
        DnsResolveTimeout = config.DnsResolveTimeout;
        _defaultDnsTimeToLive = config.DefaultDnsTimeToLive;
    }
