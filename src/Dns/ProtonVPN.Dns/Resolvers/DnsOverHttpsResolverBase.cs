...
public abstract class DnsOverHttpsResolverBase : DnsResolverBase
{
    ...
    protected DnsOverHttpsResolverBase(IConfiguration config, ILogger logger,
        IHttpClientFactory httpClientFactory, IDnsOverHttpsProvidersManager dnsOverHttpsProvidersManager,
        RecordType recordType) : base(config, logger)
    {
        _dnsOverHttpsPerProviderTimeout = config.DnsOverHttpsPerProviderTimeout;
        _dnsOverHttpsProvidersManager = dnsOverHttpsProvidersManager;
        _recordType = recordType;
        _providersUrl = config.DohProviders?.ToList() ?? new List<string>();
        _httpClient = httpClientFactory.Create();
        _httpClient.Timeout = config.DohClientTimeout;
