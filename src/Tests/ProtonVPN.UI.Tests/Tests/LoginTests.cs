...
public class LoginTests : TestSession
{
    ...
    {
        PerformLoginTest(TestUserData.FreeUser, FREE_PLAN_NAME);
    }

    [Test]
    public void LoginWithSpecialCharsUser()
    {
        PerformLoginTest(TestUserData.SpecialCharsUser, FREE_PLAN_NAME);
    }

    [Test]
    public void LoginWithTwoPassUser()
    {
        PerformLoginTest(TestUserData.TwoPassUser, FREE_PLAN_NAME);
    }

    [Test]
    public void LoginWith2FactorAuthUser()
    {
        _loginRobot.DoLogin(TestUserData.TwoFactorUser)
    ...
    {
        _loginRobot.DoLogin(TestUserData.IncorrectUser)
            .VerifyLoginErrorIsDisplayed(INCORRECT_CREDENTIALS_MESSAGE);
    }

    public void PerformLoginTest(TestUserData user, string planName)
    {
        _loginRobot.DoLogin(user);
        _homeRobot.DoWaitForVpnStatusSubtitleLabel()
            .DoOpenAccount()
